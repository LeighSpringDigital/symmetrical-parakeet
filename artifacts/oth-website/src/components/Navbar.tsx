  ...
  return (
    <>
      <nav
        ...
        style={{ height: "72px" }}
      >
        <div className="h-full flex items-center px-6 md:px-10 max-w-[1600px] mx-auto gap-8">

          {/* LEFT — Pub title */}
          <Link href="/" className="flex-shrink-0 leading-tight">
            <span className="block text-[9px] font-black tracking-[0.45em] text-gold uppercase">The Old</span>
            <span className="block font-black text-gold sc" style={{ fontSize: "1.1rem", letterSpacing: "0.18em" }}>
              Tigers Head
            </span>
          </Link>

          {/* CENTRE — All nav links grouped together */}
          <div className="hidden lg:flex items-center gap-7 flex-1 justify-center">
            {NAV_LINKS.map(l => (
              <Link
                key={l.label}
                href={l.href}
                className="text-sm font-black text-white hover:text-gold transition-colors whitespace-nowrap sc-sm uppercase"
              >
                {l.label}
              </Link>
            ))}
          ...
          <div className="hidden lg:flex items-center gap-4 flex-shrink-0">
            ...
            <Link href="/book"
              className="ml-1 bg-gold text-navy px-6 py-2 font-black text-sm tracking-[0.15em] uppercase hover:bg-white transition-colors sc-sm">
              Book
            </Link>
          </div>
          ...
          <div className="lg:hidden flex items-center justify-between w-full">
            <Link href="/" className="leading-tight">
              ...
              <span className="block text-sm font-black tracking-[0.15em] text-gold sc">Tigers Head</span>
            </Link>
            <div className="flex items-center gap-3">
              <Link href="/book" className="bg-gold text-navy px-4 py-2 font-black text-xs tracking-widest uppercase sc-sm">Book</Link>
